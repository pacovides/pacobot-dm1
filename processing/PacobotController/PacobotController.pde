...
//define the commands for our robot
final String CMD_POS = "pos";
final String CMD_LED = "led";
...
String getMessageFromCentral() {
  ...
  if (incomingMessage != null) {
    ...
    println( "Central sent:" + incomingMessage);
...
void processStringMessage(String incomingMsg) {
  ...
  if (incomingMsg==null || incomingMsg.isEmpty()) {
    println("incoming message is empty for processing");
    return;
  }

  //We assume every message is parseable in a network message
  NetworkMessage message = new NetworkMessage(incomingMsg);
  //For now, in the controller we are only interested to know if there is a command message
  if (message.messageType.equals(NetworkMessage.COMMAND)) {

    String[] command = split(message.body.trim(), '=');
    if (command.length != 2) {
      println("Invalid command " +  message.body);
      return;
    }

    if (CMD_POS.equals(command[0])) {
      processPositionCommand(command[1]);
    } else if (CMD_LED.equals(command[0])) {      
      processLedCommand(command[1]);
    } else {
...
void processLedCommand(String command) {
  if ("on".equals(command)) {
    ...
    println("invalid value for command " + CMD_LED +":" +  command);
    return;
  }
}
...
void sendPosToServer(int x, int y)
{
  ...
  NetworkMessage posMsg = new NetworkMessage(NetworkMessage.COMMAND, "101", "pos=" + x+","+y );
  client.write(posMsg.serializedMsg);
}
